...
- [ ] 文生图：`size=1024x1024` → 1k 档；`2048x1152` → 2k；`2048x2048` → 4k
- [ ] `size=auto` / 不传 → 1k 档
- [ ] **图生图（multipart）**：传 `size=2048x1152` → **2k 档**（改动前是 1k，这是本次核心验证点）
- [ ] 图生图传 `n=4` → 按 4 张计费（改动前恒按 1 张）
- [ ] gemini 原生 `generateContent` + `imageConfig.imageSize=4K` → 4k 档
- [ ] 每档扣费额与价格表一致，**且均不为 0**（为 0 说明单位写错，立即回滚）
