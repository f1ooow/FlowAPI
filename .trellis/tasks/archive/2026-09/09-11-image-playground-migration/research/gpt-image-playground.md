...
| 维度 | 选择 | 证据 |
|---|---|---|
| 框架 | React 19.1（函数组件 + hooks，无 class） | `package.json` `react ^19.1.0` |
| 构建 | **Vite 6**（`vite.config.ts`，`base: './'`，`build = tsc -b && vite build`） | `vite.config.ts:1-90` |
| 语言 | TypeScript 5.8，`target ES2020`，`strict: true`，`verbatimModuleSyntax` | `tsconfig.json` |
| 样式 | **Tailwind CSS 3.4**（`darkMode: 'media'`，`gray` 被 remap 成 `zinc`，扩展了 CSS 变量色板） | `tailwind.config.js`、`src/index.css`（488 行） |
| 状态管理 | **Zustand 5** + `persist` middleware（localStorage 快照） | `src/store.ts:491-1019` |
| 本地持久化 | **IndexedDB**（原生 API 封装，无 idb 库），DB `gpt-image-playground` v3，4 个 store：`tasks`/`images`/`thumbnails`/`agentConversations` | `src/lib/db.ts:1-348` |
| Markdown 渲染 | `react-markdown`+`remark-gfm`+`streamdown`+`@streamdown/math`+`katex` | `src/components/MarkdownRenderer.tsx:1-224`、`src/main.tsx:5-7` |
| ZIP 导出 | `fflate` | `src/lib/exportZip.ts:1-325` |
| 图标 | **自绘 SVG**（`src/components/icons.tsx`，235 行） | 无图标库依赖 |
| 字体 | CSS `@import` 外部 CDN（`fontsapi.zeoseven.com`、`jsdelivr` 的 `@lobehub/webfont-harmony-sans-sc`） | `src/index.css:1-2` |
| 包管理器 | **npm**（有 `package-lock.json`，`AGENTS.md` 明确禁止 yarn/pnpm） | `AGENTS.md` |
| 测试 | Vitest 4（`vitest run`）+ `jsdom` | `package.json` devDeps |
| PWA | 有 `manifest.webmanifest` + `sw.js`，`main.tsx` 里注册 Service Worker | `src/main.tsx:11-22` |
| 部署 | Vercel / GitHub Pages / Cloudflare Workers（wrangler）/ Docker（nginx:alpine + 启动脚本 sed 注入占位符） | `vercel.json`、`wrangler.jsonc`、`deploy/Dockerfile`、`deploy/inject-api-url.sh` |
...
`__APP_VERSION__`、`__DEV_PROXY_CONFIG__` 由 Vite `define` 注入；`import.meta.env` 读取 `VITE_DEFAULT_API_URL`、`VITE_API_PROXY_AVAILABLE`、`VITE_API_PROXY_LOCKED`、`VITE_DOCKER_DEPLOYMENT`、`VITE_SHOW_PRESET_CONFIG_ONLY`、`VITE_LOCK_PRESET_CONFIG_PARAMS`、`VITE_PREVENT_PRESET_CONFIG_DELETION`。`vite.config.ts` 还会把 `VITE_DEFAULT_API_URL` 指向的本地/远程 JSON 读取成 base64 内嵌（`embedDefaultConfig`）。
...
`src/App.tsx`（163 行）是唯一页面级组件，两处模式分支：

- `appMode === 'agent'` → `<AgentWorkspace />`（1053 行）
- 否则（**gallery**，即本次迁移要保留的「生图工作台」）→ `<main>` 内 `<SearchBar />` + `<TaskGrid />`（或 `filterFavorite && !activeFavoriteCollectionId` 时 `<FavoriteCollectionsView />`）

**两个模式共用同一个 `<InputBar />`（底部输入/参数面板）**，并且**总是**在末尾挂载以下浮层（`App.tsx:150-160`）：

`InputBar` / `DetailModal` / `Lightbox` / `SettingsModal` / `ConfirmDialog` / `SupportPromptModal` / `FavoriteCollectionPickerModal` / `ManageCollectionsModal` / `Toast` / `MaskEditorModal` / `ImageContextMenu`

`App.tsx:34-124` 的 `useEffect` 做启动引导：读 URL 查询参数、加载预置配置 JSON、`initStore()`、`setPresetConfig()`、把 URL 参数合并进 settings 再 `clearAppliedUrlSettings()`。
...
`useCloseOnEscape.ts:53`、`useDockerApiUrlMigrationNotice.ts:30`、`useDragSelect.ts:241`（框选）、`useHintTooltip.ts:68`、`usePreventBackgroundScroll.ts:127`、`useTooltip.ts:90`、`useVersionCheck.ts:72`（GitHub Release 版本检查）。
...
### 4.2 「API」tab 的字段（`SettingsModal.tsx:1421-1727`，编号即源码注释）

| # | 字段 | 说明 | 落到 `ApiProfile` |
|---|---|---|---|
| 1 | 配置名称 | :1421 | `name` |
| 2 | 服务商类型 | :1434 `<Select>`：OpenAI 兼容接口 / sub2api（异步）/ fal.ai / 自定义服务商 | `provider` |
| 3 | **API URL** | :1447，placeholder 提示会自动补 `/v1`；末尾带 `/` 则不补 | `baseUrl` |
| 4 | **API 代理** | :1474，开关；部署端 `VITE_API_PROXY_LOCKED` 时锁定 | `apiProxy` |
| 5 | **API Key** | :1499，password 输入 + 显隐切换 | `apiKey` |
| 6 | API 接口 | :1537 `<Select>`：Images / Responses | `apiMode` |
| 7 | **模型 ID** | :1564，自由文本 | `model` |
| — | 推理强度 | :1597，7 档（none…max） | `reasoningEffort` |
| 8 | 流式传输 + 中间步骤图像数 | :1617，开关 + 0–3 | `streamImages` / `streamPartialImages` |
| 9 | 返回 Base64 图片数据 | :1664，追加 `response_format: b64_json` | `responseFormatB64Json` |
| 10 | Codex CLI 兼容模式 | :1687 | `codexCli` |
| 11 | 请求超时（秒） | :1710 | `timeout`（默认 600） |
| — | 配置列表 | 下拉可切换/复制/删除/拖拽排序；「复制导入配置 URL」 | `profiles` / `activeProfileId` / `providerOrder` |

### 4.3 其他 4 个 tab

- **通用（GeneralSettingsTab.tsx）**：任务提交方式（Enter / Ctrl+Enter，移动端为发送按钮）、提交后清空输入框、ZIP 批量下载途径（`zipDownloadRoutes` 6 条途径）、重启后加载上次输入框、复用配置时临时复用该任务的 API 配置、成功任务仍显示重试按钮、允许模型改写提示词、任务完成系统通知、Agent 自动滚底、公式输出提示。
- **Agent（AgentSettingsTab.tsx）**：`agentApiConfigMode`（off/native/hybrid）、`agentTextProfileId`、`agentImageProfileId`、`agentMaxToolRounds`、`agentWebSearch`。——**整体删**。
- **数据（data）**：全量导出/导入 ZIP（`exportData` / `importData`，`store.ts:4356` 起）、清空数据（`clearData`），勾选项 `exportConfig/exportTasks/importConfig/importTasks/clearConfig/clearTasks`。
- **关于（about）**：版本号（`__APP_VERSION__`）、GitHub 链接。

### 4.4 存储位置

| 内容 | 存储 | 位置 |
|---|---|---|
| `settings`（含 `profiles[]`、`apiKey`、`baseUrl`、`model`、`apiProxy`、所有开关） | **localStorage**，zustand `persist` | `store.ts:1013-1019`：`{ name: 'gpt-image-playground', version: 2, migrate, partialize: getPersistedState, merge: mergePersistedState }` |
| 任务记录 `TaskRecord` | IndexedDB `gpt-image-playground` v3，store `tasks` | `lib/db.ts:3-8` |
| 图片（dataURL/Blob）、缩略图 | IndexedDB `images` / `thumbnails` | `lib/db.ts` |
| Agent 对话 | IndexedDB `agentConversations`（老版本在 localStorage，有迁移逻辑） | `lib/persistedState.ts:96`、`lib/agentResponseState.ts` |
| 配置导入 URL 弹窗的勾选状态 | 独立 localStorage key `gpt-image-playground.copy-import-url-options` | `SettingsModal.tsx:69-112` |
...
### 5.4 Responses API（`callResponsesImageApi`，`openaiCompatibleImageApi.ts:986-1055`）

```
POST {baseUrl}/v1/responses
{
  model, input: [...], tools: [{ type: 'image_generation', action: 'generate'|'edit',
    output_format, moderation, size, quality, partial_images, output_compression,
...
结果从 `output[].type === 'image_generation_call'` 的 `result.b64_json / base64 / image / data` 取（`imageApiShared.ts:50-70`）。

### 5.5 是否 OpenAI Images API 兼容

**是。** 请求路径、请求体字段名（`model` / `prompt` / `size` / `quality` / `output_format` / `output_compression` / `moderation` / `n` / `response_format`）完全按 OpenAI Images API 形态设计，并额外支持 gpt-image 系列的 `output_format` / `output_compression` / `moderation` / `partial_images` / `transparent_output`（后者是提示词工程 + 前端抠图后处理，见 `lib/transparentImage.ts:1-80`）。多出的是 `stream` + `partial_images` 与 `response_format: b64_json` 两个可选开关。

### 5.6 流式

支持，但**仅在 profile 开启**时：`stream: true` + `partial_images: 0..3`，响应按 SSE 解析（`lib/serverSentEvents.ts`，`readJsonServerSentEvents`）。事件名（`openaiCompatibleImageApi.ts:290-316`）：
...
注：FlowAPI relay 侧有对应的 SSE 处理（`relay/helper/stream_gate.go:152/163` 识别 `b64_json` / `partial_image_b64` / `result`，`relay/channel/openai/image_stream_test.go` 断言 `image_generation.partial_image` 事件），`relay/channel/openai/image_edit_test.go:32/67` 也覆盖了 `partial_images` 表单字段透传，所以流式链路后端是通的。

### 5.7 图片返回格式

三种都处理（`parseImagesApiResponse`，`openaiCompatibleImageApi.ts:218-266`）：

1. `data[].b64_json` → `normalizeBase64Image` 加 `data:{mime};base64,` 前缀
2. `data[].url` / `data:...` → `fetchImageUrlAsDataUrl` 抓回来转 dataURL（并记录 `rawImageUrls`）
3. 流式的 `completedItems` 同上处理

解析失败时把原始响应挂到 `err.rawResponsePayload`（`:190/211/772/910`），用于 UI 展示「原始响应」（`TaskRecord.rawResponsePayload`）。实际生效参数从响应顶层 `size/quality/output_format/output_compression/moderation/n` 提取（`pickActualParams` / `mergeActualParams`）。
...
| 依赖 | 用途 | 被谁用 | 迁移建议（事实陈述） |
|---|---|---|---|
| `fflate` ^0.8.3 | ZIP 打包/解包 | `lib/exportZip.ts`、SettingsModal 数据 tab | 只服务导出/导入功能 |
| `react-markdown` ^10.1.0 + `remark-gfm` ^4.0.1 + `streamdown` ^2.5.0 + `@streamdown/math` ^1.0.2 | Markdown/公式渲染 | `components/MarkdownRenderer.tsx`、`main.tsx:5-7` | FlowAPI 已有 `marked` + `katex` + `dompurify`（`web/src/components/ui/markdown.tsx`），功能重叠 |
| `@fal-ai/client` ^1.10.0 | fal.ai SDK | `lib/falAiImageApi.ts` | FlowAPI 无 fal 供应商需求 |
| `core-js` ^3.49.0 | `Array.prototype.at` polyfill | `main.tsx:1` | ES2020 target 下的历史包袱 |
| `katex` ^0.16.47 | 公式渲染 | `MarkdownRenderer` | FlowAPI 有 `katex ^0.17.0`（版本不同） |
| `zustand` ^5.0.5 | 状态管理 | 全局 store | FlowAPI 有 `zustand ^5.0.14`（**版本兼容**） |
| `react` / `react-dom` ^19.1.0 | — | — | FlowAPI 是 ^19.2.7（兼容） |
| `tailwindcss` ^3.4.17 | — | — | **FlowAPI 是 Tailwind v4**（`^4.3.2` + `@rsbuild/plugin-tailwindcss`），这是最大的样式适配点 |
| `vite` / `@vitejs/plugin-react` / `wrangler` | 构建 | — | FlowAPI 用 Rsbuild 2 |

### 反向：FlowAPI 有、原项目没有的（可用于替换）

`@base-ui/react`、`@tanstack/react-router` / `react-query`、`i18next` / `react-i18next`、`lucide-react`、`sonner`（toast）、`clsx` / `tailwind-merge` / `class-variance-authority`、`marked` + `katex` + `dompurify`、`vaul`、`motion`、`react-hook-form` + `zod`。
